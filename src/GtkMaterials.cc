...
void GtkMaterials::CreateMaterials()
{
  ...
  // Scintillator
  //--------------------------------------------------
  fScintillator = fNistMan->ConstructNewMaterial("Scintillator", elements, natoms, density);
  ...
  MPTscint->AddProperty("ABSLENGTH", scintABSEnergy,      scintABSLength,  502);
	...
	MPTscint->AddConstProperty("SCINTILLATIONYIELD",  12/MeV);
  ...
  MPTscint->AddConstProperty("ReemissionFASTTIMECONSTANT",  1.50*ns);
	...
	fScintillator->SetMaterialPropertiesTable(MPTscint);

	fScintillator->GetIonisation()->SetBirksConstant(0.15*mm/MeV);  
